...
void dice(int time){
	...
	for(int i=2;i<=12;++i){
		printf("%2d %6d      %7.2f%%      %7.2f%%     %7.2f%%\n"
				,i,stat[i],(double)stat[i]*100/time,(double)expect[i]*100/36,
				 std::fabs((double)stat[i]*100/time-(double)expect[i]*100/36));
		sum += std::fabs((double)stat[i]*100/time-expect[i]*100/36);
	}
